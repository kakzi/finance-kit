...
class CreateReturnPembelian extends CreateRecord
{
    ...
    protected function afterCreate(): void
    {
        ...
        $levels = LevelApproval::where('limit_amount', '>=', $cashIn->total_return_pembelian)
            ->where('level', '!=', 'L0')
            ->get();
        // dd($levels);
        if (! $levels) {
        ...
        $approvalRequest = ApprovalRequest::create([
            'module'        => 'return-pembelian',
            'module_id'     => $cashIn->id,
            'amount'        => $cashIn->total_return_pembelian,
            'requested_by'  => auth()->id(),
            'approval_by'   => $approvers,
            'status'        => 'pending',
            'note'          => 'Return Pembelian',
        ]);
       ...
       // 4️⃣ Kirim notifikasi WhatsApp ke semua approver
        ...
        foreach ($approvers as $approverId) {
            ...
            $pesan = 
                ...
                "🧾 Modul: *Return Pembelian*\n" .
                "🧾 Nomor Faktur: *".$cashIn->nomor_faktur."*\n" .
                "🏢 Tangal: *" . Carbon::parse($cashIn->tanggal)
                    ->translatedFormat('l, d F Y') . "*\n" .
                "🏢 Total: Rp *" . number_format($cashIn->total_return_pembelian, 0, ',', '.') . "*\n" .
