...
class CreateMutasiBarang extends CreateRecord
{
    ...
    protected function afterCreate(): void
    {
        ...
        $levels = LevelApproval::where('limit_amount', '>=', $cashIn->total_mutasi)
            ->where('level', '!=', 'L0')
            ->get();
        // dd($levels);
        if (! $levels) {
        ...
        $approvalRequest = ApprovalRequest::create([
            'module'        => 'mutasi_barang',
            'module_id'     => $cashIn->id,
            'amount'        => $cashIn->total_mutasi,
            'requested_by'  => auth()->id(),
            'approval_by'   => $approvers,
            'status'        => 'pending',
            'note'          => 'Permintaan Persetujuan Mutasi Barang',
        ]);
       ...
       // 4️⃣ Kirim notifikasi WhatsApp ke semua approver
        ...
        foreach ($approvers as $approverId) {
            ...
            $pesan = 
                ...
                "🏢 Tangal Mutasi: *" . Carbon::parse($cashIn->tanggal_mutasi)
                    ->translatedFormat('l, d F Y') . "*\n" .
                "🏢 Tangal Kirim: *" . Carbon::parse($cashIn->tanggal_mutasi)
                    ->translatedFormat('l, d F Y') . "*\n" .
                "🏢 Dari Kantor: *" . $cashIn->officeFrom->name . "*\n" .
                "🏢 Kantor Tujuan: *" . $cashIn->officeTo->name . "*\n" .
                "🏢 Driver dan Helper: *" . $cashIn->driver_helper . "*\n" .
                "🏢 Transportasi: *" . $cashIn->transportasi->name . "*\n" .
                "🏢 Checker: *" . $cashIn->checker . "*\n" .
                "🏢 Nama Penerima: *" . $cashIn->recipient . "*\n" .
                "🏢 Total Mutasi: Rp *" . number_format($cashIn->total_mutasi, 0, ',', '.') . "*\n" .
